...
*  Project : Boxcar
*
*  Description : A cache being able to download, resize and store images
*
*  DRI     : Paul LEMAIRE
*  Created : 01/12/2014
*  Copyright (c) 2014 Process-one. All rights reserved.
*
*/
...
class PLMImageCache {
    ...
    func localDataForKey(key: String, andSize desiredSizeImage: CGSize) -> NSData? {
        ...
        let imagePath = String(format: "%0.fx%0.f", desiredSizeImage.width, desiredSizeImage.height)
    ...
    func writeLocalData(data: NSData, forKey key: String, withSize desiredImageSize:CGSize) {
        let imagePath = String(format: "%0.fx%0.f", desiredImageSize.width, desiredImageSize.height)
        
        var cachePath = self.pathToContainerDictionary[imagePath]
        //If the folder does not exist this means this is a new size
        if cachePath == nil {
            cachePath = NSSearchPathForDirectoriesInDomains(.CachesDirectory, .UserDomainMask, true)[0].stringByAppendingPathComponent(String(format: "/images/resized/%0.fx%0.f", desiredImageSize.width, desiredImageSize.height))
    ...
    func imageWithData(data: NSData?, convertToSize size:CGSize) -> NSData? {
        ...
        if let tmpData = data as NSData! {
            ...
            let imageData = UIImageJPEGRepresentation(finalImage, 1)//1 is best quality, 0 is poor quality
            return imageData;
        } else {
            return nil
        }
